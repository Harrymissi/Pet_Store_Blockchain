...
import Provicne from '../dbs/models/province'
...
router.get('/getPosition', async (ctx) => {
  ...
  } = await axios.get(`http://cp-tools.cn/geo/getPosition?sign=${sign}`); //第三方服务
...
router.get('/province', async (ctx) => {
  // let province = await Provicne.find();
  // ctx.body = {
  //   province: province.map(item => {      //从数据库里拿数据
  //     return {
  //       id: item.id,
  //       name: item.value[0]
  //     }
